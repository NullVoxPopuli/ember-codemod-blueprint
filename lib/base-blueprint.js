 ...
 *
 * TODO: add -b argument to generate command in ember-cli so that
 *       we can either use execa to generate any blueprint by path,
 *       and/or use the JS API for -b so that we don't have to read
 *       stdout in order to achieve the above
 */
module.exports = {
  ...
  async _installUpstreamBlueprint(options) {
    ...
    let blueprint = path.join(process.cwd(), 'node_modules', this.upstream).toString();
    let blueprintParts = this.upstream.split('/');

    blueprint = blueprintParts[blueprintParts.length - 1];

    console.log('blueprint path', blueprint);
